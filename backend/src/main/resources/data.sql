...
INSERT INTO users (
    ...
    -- 서대문구 법정동 코드(1141000000). 강남구(1168000000)에서 바꿨다 — 용산구(1117000000)와
    -- 함께 지역코드 매칭 API가 붙을 때 쓸 실제 코드 두 개 중 하나라, 데모 계정도 실제 코드로 맞춘다.
    '2025-03-01', '1141000000', '서울특별시', '서대문구',
...
--
-- 연동 기관은 4곳으로 고정한다: 국민은행(계좌) · 현대캐피탈(대출) · 국민카드 · 우리카드.
-- 마이데이터 연동 화면에서 사용자가 어떤 기관을 고르든 실제로 붙는 건 이 4곳뿐이다
-- (연동은 통째로 데모 계정을 복제하는 구조라 선택 자체가 결과에 영향을 주지 않는다).
--
-- org_code: 은행은 표준 금융기관 코드(국민 004), 카드사·캐피탈은 목값이다.
...
-- ⚠️ 연동 기관을 4곳(국민은행·현대캐피탈·국민카드·우리카드)으로 좁히면서 신한 계열
-- (계좌·대출·카드)을 뺐다. data.sql은 INSERT/UPDATE만 하고 DELETE를 못 하므로,
-- 이미 이 행들을 시드해둔 로컬·운영 DB에도 반영되도록 여기서 명시적으로 지운다.
-- 자식 행(거래내역)을 FK 때문에 먼저 지운다. **거래 id를 하드코딩하지 않고 계좌·카드
-- id로 지운다** — data.sql은 예전 버전에서도 계속 INSERT만 해왔으므로, 지금 이 파일에
-- 안 보이는 옛 거래 행이 이미 DB에 더 쌓여 있을 수 있다. id를 몇 개만 콕 집어 지우면
-- 그 나머지가 고아로 남아 FK 위반으로 기동이 깨진다.
DELETE FROM mock_mydata_account_transaction WHERE mock_account_id IN (9003);
DELETE FROM mock_mydata_card_transaction WHERE mock_card_id = 9002;
DELETE FROM mock_mydata_account WHERE mock_account_id IN (9003);
DELETE FROM mock_mydata_card WHERE mock_card_id = 9002;

-- 계좌 3개: 국민은행 주거래 + 국민은행 청년적금 + 국민은행 투자계좌.
...
INSERT INTO mock_mydata_account (
    ...
    (9001, 9001, 'KB-ACC-0001', '004', '110-2345-678901',
     '국민은행 주거래통장', '국민은행 마이핏통장', 'DEPOSIT',  842000,  842000, NULL, 1, '2026-09-01 00:00:00', '2026-09-01 00:00:00'),
    (9002, 9001, 'KB-ACC-0002', '004', '110-2345-678902',
     '국민은행 청년적금',   '국민은행 청년도약적금', 'SAVINGS', 3600000,       0, NULL, 1, '2026-09-01 00:00:00', '2026-09-01 00:00:00'),
    (99005, 9001, 'KB-ACC-0003', '004', '110-2345-678903',
     '국민은행 투자계좌',   'KB 온국민 글로벌 투자계좌', 'INVESTMENT', 700000, 700000, NULL, 1, '2026-09-01 00:00:00', '2026-09-09 20:00:00'),
    -- 제1금융권 대출 한 건, 제2금융권 대출 한 건. 같은 금액이라도 어디서 빌렸는지가
    -- 점수를 가르므로 두 권역이 다 있어야 화면이 의미를 가진다 — 마침 연동 기관을
    -- 4곳으로 좁힌 국민은행(1금융권)·현대캐피탈(2금융권)이 그대로 맞아떨어진다.
    --
    -- 상품은 자립준비청년이 실제로 쓰는 것으로 둔다. 직장인 대상 신용대출은
    -- 이 서비스의 사용자상과 맞지 않는다.
    --   · 청년전용 버팀목전세자금대출 — 주택도시기금 상품이고 국민은행이 취급한다.
    ...
    (99001, 9001, 'KB-LOAN-0001', '004', '110-2345-500001',
     '국민은행 전세자금대출', '청년전용 버팀목전세자금대출', 'LOAN', 30000000, NULL, 2.10, 1, '2026-09-01 00:00:00', '2026-09-01 00:00:00'),
...
-- 카드 3장: 우리카드 체크 + 국민카드 신용 + 우리카드 신용.
-- 체크카드는 한도가 없어 credit_limit이 NULL이다. 신용카드만 이용률을 낼 수 있다.
--
-- 9001은 원래 현대카드 체크였다 — 기관을 4곳으로 좁히면서 우리카드로 바꿨다.
-- external_card_id에 옛 접두사(HD-)가 남아있는데, 카드 소유자를 가리키는 식별자일 뿐
-- 화면에는 org_code·card_name만 노출되므로 굳이 딸린 거래내역까지 다시 붙이지 않는다.
--
...
INSERT INTO mock_mydata_card (
    ...
    (9001, 9001, 'HD-CARD-0001', '0313', '우리카드 체크카드',
     'CHECK', NULL, 1, '2026-09-01 00:00:00', '2026-09-01 00:00:00'),
    (99003, 9001, 'KB-CARD-0001', '0301', '국민카드 탄탄대로 카드',
...
INSERT INTO mock_mydata_account_transaction (
    ...
    -- 6월 KB 주거래: 수입 1,200,000원, 계좌 지출 2,040,000원.
    -- 같은 달 카드 지출 160,000원까지 합치면 총지출 2,200,000원으로 수입보다 1,000,000원 많다.
    (9041, 9001, 'KB-TX-202606-01', '2026-06-20 09:12:00', 'INCOME',  1200000, '지에스리테일', NULL, NULL, NULL, '6월 급여',       'NORMAL', '2026-06-20 09:12:00', '2026-06-20 09:12:00'),
    (9042, 9001, 'KB-TX-202606-02', '2026-06-21 10:00:00', 'EXPENSE',  700000, 'LH청년행복주택', NULL, 'LH청년행복주택', '1298200036', '6월 주거비', 'NORMAL', '2026-06-21 10:00:00', '2026-06-21 10:00:00'),
    (9043, 9001, 'KB-TX-202606-03', '2026-06-27 18:30:00', 'EXPENSE', 1340000, '생활비 정산', NULL, NULL, NULL, '6월 생활비',       'NORMAL', '2026-06-27 18:30:00', '2026-06-27 18:30:00'),
    -- 7월 KB 주거래: 수입 1,000,000원, 계좌 지출 750,000원.
    -- 같은 달 카드 지출 150,000원까지 합치면 총지출 900,000원으로 수입보다 100,000원 적다.
    (9044, 9001, 'KB-TX-202607-01', '2026-07-20 09:12:00', 'INCOME',  1000000, '지에스리테일', NULL, NULL, NULL, '7월 급여',       'NORMAL', '2026-07-20 09:12:00', '2026-07-20 09:12:00'),
    (9045, 9001, 'KB-TX-202607-02', '2026-07-22 10:00:00', 'EXPENSE',  750000, 'LH청년행복주택', NULL, 'LH청년행복주택', '1298200036', '7월 주거비', 'NORMAL', '2026-07-22 10:00:00', '2026-07-22 10:00:00'),
    -- 기존 DB에도 같은 외부 거래 ID가 남아 있으므로 행을 제거하지 않고 OTHER로 갱신해 지출 집계에서 제외한다.
    (9046, 9001, 'KB-TX-202607-03', '2026-07-28 18:30:00', 'OTHER',    200000, '계좌 정산', NULL, NULL, NULL, '정산 이체',          'NORMAL', '2026-07-28 18:30:00', '2026-07-28 18:30:00'),
    -- 8월 KB 주거래
    ...
    (9016, 9001, 'KB-TX-202609-06', '2026-09-09 09:00:00', 'TRANSFER', 250000, '본인',         '110-2345-678902', NULL, NULL, '청년적금 자동이체', 'NORMAL', '2026-09-09 09:00:00', '2026-09-09 09:00:00'),
    ...
    (9032, 9002, 'KB-TX-202609-06-IN', '2026-09-09 09:00:00', 'TRANSFER', 250000, '본인', '110-2345-678901', NULL, NULL, '청년적금 납입', 'NORMAL', '2026-09-09 09:00:00', '2026-09-09 09:00:00')
ON DUPLICATE KEY UPDATE
...
-- 카드 거래내역 (우리카드 체크). 소비 분류·포켓 화면이 읽는 실제 소비 데이터다.
--
